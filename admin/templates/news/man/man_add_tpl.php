<?php
function get_tour($id=0)
{
    global $d;
    if($id)
    {
        $temps = $d->rawQueryOne("select id_tour from #_product where id = ? and type = ? limit 0,1",array($id,'san-pham'));
        $arr_tour = explode(',', $temps['id_tour']);
        for($i=0;$i<count($arr_tour);$i++) $temp[$i]=$arr_tour[$i];
    }
    $row_tour = $d->rawQuery("select tenvi, id from #_news where type = 'hoi-dong' order by stt,id desc");
    $str = '<select id="tour_group" name="tour_group[]" class="select multiselect" multiple="multiple" >';
    for($i=0;$i<count($row_tour);$i++)
    {
        if(isset($temp) && count($temp) > 0)
        {
            if(in_array($row_tour[$i]['id'],$temp)) $selected = 'selected="selected"';
            else $selected = '';
        }
        else
        {
        ...
        $str .= '<option value="'.$row_tour[$i]["id"].'" '.$selected.' /> '.$row_tour[$i]["tenvi"].'</option>';
    }
    $str .= '</select>';
    return $str;
}
    ...
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="row">
            ...
            <div class="<?= $colRight ?>">
                <?php if (
                    (isset($config['news'][$type]['dropdown']) && $config['news'][$type]['dropdown'] == true) ||
                    (isset($config['news'][$type]['tour']) && $config['news'][$type]['tour'] == true) ||
                    (isset($config['news'][$type]['tags']) && $config['news'][$type]['tags'] == true)
                ) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
                        ...
                        <div class="card-body">
                            <div class="form-group-category row">
                                <?php if (isset($config['news'][$type]['dropdown']) && $config['news'][$type]['dropdown'] == true) { ?>
                                    ...
                                    <?php } ?>
                                <?php } ?>
                                <?php if (isset($config['news'][$type]['tour']) && $config['news'][$type]['tour'] == true) { ?>
                                    <div class="form-group col-xl-12">
                                        <label class="d-block" for="id_tours">Danh mục tour:</label>
                                        <?= $func->getTour(@$item['id'], 'tour_group', 'news', $type) ?>
                                    </div>
                                <?php } ?>
                ...
                <?php /*if(isset($config['news'][$type]['tour']) && $config['news'][$type]['tour'] == true) { ?>
                    <div class="form-group col-xl-6 col-sm-4">
                        <label class="d-block" for="id_tour">Tour liên quan:</label>
                        <?=get_tour(@$item['id'])?>
                    </div>
                <?php } */?>
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <?php if (isset($config['news'][$type]['tuvan']) && $config['news'][$type]['tuvan'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label class="d-block" for="hotline">Hotline:</label>
                            <input type="text" class="form-control text-sm" name="data[options][hotline]" id="hotline" placeholder="Hotline" value="<?= $options['hotline'] ?>">
                        </div>
                    <?php } ?>
                    <?php if (isset($config['news'][$type]['tuvan2']) && $config['news'][$type]['tuvan2'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label class="d-block" for="goingay">Gọi ngay:</label>
                            <input type="text" class="form-control text-sm" name="data[options][goingay]" id="goingay" placeholder="Gọi ngay" value="<?= $options['goingay'] ?>">
                        </div>
                        <div class="form-group col-md-4">
                            <label class="d-block" for="zalo">Chat zalo:</label>
                            <input type="text" class="form-control text-sm" name="data[options][zalo]" id="zalo" placeholder="Chat zalo" value="<?= $options['zalo'] ?>">
                        </div>
                    <?php } ?>
                </div>
