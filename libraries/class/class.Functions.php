...
class Functions
{
    ...
    /* Get tags */
    public function getTour($id = 0, $element = '', $table = '', $type = '')
    {
        if ($id) {
            $temps = $this->d->rawQueryOne("select id_tours from #_" . $table . " where type = ? and id = ?", array($type,$id));
            $arr_tours = explode(',', $temps['id_tours']);
            for($i=0;$i<count($arr_tours);$i++) $temp[$i]=$arr_tours[$i];
        }

        $row_tags = $this->cache->get("select namevi, id from #_product where type = ? order by numb,id desc", array('san-pham'), "result", 7200);
