...
function saveMan()
{
    ...
    // $dataTours = (!empty($_POST['dataTours'])) ? $_POST['dataTours'] : null;
    $row = $d->rawQueryOne("select id, options from #_news where id = ? limit 0,1", array($id));
    ...
    if ($data) {
        ...
        /* Tours */
        if (isset($config['news'][$type]['tour']) && $config['news'][$type]['tour'] == true) {
            if ($data) {
                $data['id_tours'] = implode(",", $_POST['tour_group']);
            } else {
                $data['id_tours'] = "";
            }
        }
