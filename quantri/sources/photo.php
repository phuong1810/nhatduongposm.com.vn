    ...
    if ($id) {
        ...
        if ($d->update('photo', $data)) {
            ...
            if ($func->hasFile("file2")) {
                ...
                if ($photo2 = $func->uploadImage("file2", $config['photo']['man_photo'][$type]['img_type_photo'], UPLOAD_PHOTO, $file_name2)) {
                    $row = $d->rawQueryOne("select id, photo2 from #_photo where id = ? and type = ? limit 0,1", array($id, $type));
            ...
            for ($i = 0; $i < count($dataMultiTemp); $i++) {
                ...
                if (($success_photo || $success_video) && empty($response)) {
                    if ($d->insert('photo', $dataMulti)) {
                        ...
                        /* Photo2 */
                        if (isset($config['photo']['man_photo'][$type]['images_photo']) && $config['photo']['man_photo'][$type]['images_photo'] == true) {
                            if ($func->hasFile("file2" . $i)) {
                                $photoUpdate2 = array();
                                $file_name2 = $func->uploadName($_FILES["file2" . $i]["name"]);

                                if ($photo2 = $func->uploadImage("file2" . $i, $config['photo']['man_photo'][$type]['img_type_photo'], UPLOAD_PHOTO, $file_name2 . $i)) {
                                    $photoUpdate2['photo2'] = $photo2;
                                    $d->where('id', $id_insert);
                                    $d->update('photo', $photoUpdate2);
                                    unset($photoUpdate2);
...
function deletePhoto()
{
    ...
    if ($id) {
        $row = $d->rawQueryOne("select id, photo, photo2 from #_photo where id = ? and type = ? limit 0,1", array($id, $type));

        if (!empty($row)) {
            $func->deleteFile(UPLOAD_PHOTO . $row['photo']);
            $func->deleteFile(UPLOAD_PHOTO . $row['photo2']); // Xóa luôn photo2
