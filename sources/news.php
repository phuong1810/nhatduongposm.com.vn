...
if ($id != '') {
    /* Lấy bài viết detail */
    $rowDetail = $d->rawQueryOne("select id_tours,id, view, date_created, id_list, id_cat, id_item, id_sub, type, name$lang, slugvi, slugen, desc$lang, content$lang, photo, options from #_news where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));
    /* Lấy tour */
    if($rowDetail['id_tours']) $tourslq = $d->rawQuery("select id, name$lang, slugvi, slugen, desc$lang, content$lang, code, view, id_brand, id_list, id_cat, id_item, id_sub, photo, options, discount, sale_price, regular_price,songay, songuoi, diadiem,sosao,review,bando,huongdan$lang,chiphidukien$lang from #_product where type='san-pham' and find_in_set(id,'".$rowDetail['id_tours']."') and find_in_set('hienthi',status)");
