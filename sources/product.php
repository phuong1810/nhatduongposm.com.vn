...
if ($id != '') {
    ...
    $optdetail = (!empty($rowDetail['options'])) ? json_decode($rowDetail['options'], true) : null;
    ...
    $productDesc = $d->rawQueryOne("select id, name$lang, slugvi, slugen, type, photo, options,content$lang from #_product_list where id = ? and type = ? limit 0,1", array($idl, $type));
